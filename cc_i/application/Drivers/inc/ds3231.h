...
HAL_StatusTypeDef DS3231_Write_Byte(uint8_t REG_ADD,uint8_t dat);	//DS3231某寄存器写入一个字节数据

uint8_t DS3231_Read_Byte(uint8_t REG_ADD);	//DS3231某寄存器读取一个字节数据


void DS3231_ReadWrite_Time(Time_Typedef * TimeValue, uint8_t mode);	//DS3231读取或者写入时间信息

void DS3231_Time_Init(Time_Typedef *TimeVAL);	//时间日历初始化

void DS3231_Get_Date_Time(Time_Typedef *TimeValue);	//时间日历数据处理函数
